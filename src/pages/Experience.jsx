...
function Experience(props) {
    ...
    return (
        <>
            <div id={"experience"} className={`flex flex-col ${isMobile?"gap-1":"gap-4"} justify-center items-center`}>
                <div>
                    <h3 className={styles.skill_title+` text-3xl font-bold ${props.backConfig==="dark"?"white":"black"}`}>Experience</h3>
                </div>
                <div style={{padding:"1rem"}} className={"flex flex-col gap-6 rounded-2xl border-[1px] border-amber-500"}>
                    ...
                    <h3 className={`font-bold ${isMobile?"text-lg":"text-3xl"} border-b-amber-500 border-b-[0.5px] ${props.backConfig==="dark"?"white":"black"}`}>학습</h3>
                        <div className={`grid grid-cols-[1fr_1fr_3fr] gap-4 ${props.backConfig==="dark"?"white":"black"} ${isMobile?"text-sm":"text-lg"}`}>
                            <h3>그린컴퓨터아트학원</h3>
                    ...
                    <div className={"flex flex-col gap-4 shadow-2xs"}>
                        <h3 className={`font-bold ${isMobile?"text-lg":"text-3xl"} border-b-amber-500 border-b-[0.5px] ${props.backConfig==="dark"?"white":"black"}`}>실무</h3>
                        <div className={`grid grid-cols-[1fr_1fr_3fr] gap-4 ${props.backConfig==="dark"?"white":"black"} ${isMobile?"text-sm":"text-lg"}`}>
                            <h3>㈜이젠솔루션</h3>
